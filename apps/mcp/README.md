...
| `list_promises` | _(none)_ | account | Lists every promise on the account (pending, active, or resolved) with remaining budget, categories, expiry, and replacement lineage (`replaces`/`replacedBy`). |
...
**A promise can evolve mid-task, but only the human can change it:** if the
human changes what they want — the item is unavailable, they pick an
alternative, a different budget or store — the agent must never keep trying
to pay under the old promise. Instead it calls `request_promise` again with
`replaces` set to the current promiseId, describing exactly what the human
now wants; the human approves that specific change (old task/budget →
new task/budget) in World App, and only on approval does the old promise
stop working — a denied or expired replacement leaves it untouched. Jev
still checks every payment against whichever promise is currently approved,
never the conversation, so a payment that no longer matches the (old)
promise is refused with an `actionableHint` pointing the agent at
`request_promise`'s `replaces` — but only for that specific refusal reason;
every other refusal (funding, provenance, Intercepta, policy) is unaffected
and never suggests a replacement.
