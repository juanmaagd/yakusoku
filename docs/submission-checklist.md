...
| Git used throughout, frequent commits (not one giant commit) | ✅ | `git log --oneline` shows 39+ commits, one or more per work unit, Conventional Commits style. |
...
| AI assists but does not build the project alone | ✅ | `docs/ai/README.md` "Who did what" — every product/architecture decision (Jev calibration policy, agent model, demo scope, pipeline reordering, agent-first positioning) attributed to the human; independent verification step documented for every WU. |
| Spec-driven flow artifacts (specs/prompts/planning) live in the repo | ✅ | Copied to `docs/ai/planning/` (spike reports excluded, see its README), including the execution roadmap (`docs/ai/planning/roadmap.md`, kept in sync with the live `odd/tasks/yakusoku.md`). |
...
| Demo video: 2–4 minutes | ⏳ | Not yet recorded. Script drafted at `docs/demo-script.md`, targeted 2:30–3:30, agent-first (an MCP-connected agent drives the purchase, not a browser wallet screen). |
...
| Demonstrate the complete journey: request → user completion → validated result → protected action | ⏳ | Fully implemented and covered by the automated `bun run scenarios` suite against the real sandbox. The device flow itself (`user_code`/`verification_uri`) was verified live during pre-hackathon planning with a real World App approval, and again on 2026-09-26 with a genuine MCP client (Claude Code, `apps/mcp` over stdio) reaching a real `needs_human_approval` verdict with a live sandbox `verificationUri`/user code. A live World App tap completing that protected action (signing a real payment) is still pending — no approval was given during either run, by design. |
...
| Validate identity results server-side; no client secrets exposed | ✅ | `validateIdToken` (`apps/firewall/world-id.ts:260`) runs entirely in the firewall process using `jose` + remote JWKS. `WORLD_CLIENT_ID`/`WORLD_CLIENT_SECRET` are read only by the firewall; the site and the agent never receive them. |
...
| Requirement (verbatim, paraphrased) | Status | Evidence |
|---|:---:|---|
| Public GitHub repo with project artifacts (contracts/tests/docs) and a solid README | ✅ (no custom contracts) | No Solidity contracts were written (deliberately out of scope — the firewall signs standard x402/USDC payments, no custom on-chain logic); tests (`bun test`, 135 passing) and documentation are present. |
...
- ✅ Done: 16
- ⏳ Pending (human action, mostly "arrives tomorrow" items already known to the team): 10
- ❌ Gap (needs a decision, not just a live check): 1 — Discord-pinned Intercepta known-risk test addresses never obtained (public OFAC address used as a substitute).
